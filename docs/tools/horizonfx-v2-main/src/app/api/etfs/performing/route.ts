...
export async function GET() {
  try {
    ...
    // Generate cache key for performing ETFs
    const cacheKey = generateCacheKey('etfs:performing');
    ...
    const apiUrl = `${API_BASE_URL}/etfs/top-performing?count=25`;
    ...
    console.error('Error fetching top performing ETFs:', error);
    return NextResponse.json(
      { error: 'Failed to fetch top performing ETFs data' },
      { status: 500 }
    );
  }
}