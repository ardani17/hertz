...
const copy = {
  id: {
    ...
    rulesGroup: { balance: 'Saldo & akun', target: 'Target profit', risk: 'Batas risiko', evaluation: 'Aturan evaluasi' },
    showAllMetrics: 'Lihat semua metrik',
    hideAllMetrics: 'Sembunyikan metrik tambahan',
    filterAll: 'Semua',
    mockBannerTitle: 'Mode demo — AI Review',
  },
  en: {
    tabs: { overview: 'Overview', rules: 'Rules', journal: 'Journal', analytics: 'Analytics', risk: 'Risk Monitor', ai: 'AI Review' },
    ...
    rulesGroup: { balance: 'Balance & account', target: 'Profit target', risk: 'Risk limits', evaluation: 'Evaluation rules' },
    showAllMetrics: 'Show all metrics',
    hideAllMetrics: 'Hide extra metrics',
    filterAll: 'All',
    mockBannerTitle: 'Demo mode — AI Review',
  },
};
...
export function ChallengeTrackerTool() {
  ...
  return (
    <section className={styles.challengeShell}>
      ...
      {tab === 'overview' && selected && effectiveAccount && overview && riskStatus ? (
        <Overview t={t} account={effectiveAccount} analytics={analytics} overview={overview} riskStatus={riskStatus} trades={trades} />
      ) : null}
...
function Overview({ t, account, analytics, overview, riskStatus, trades }: { t: Copy; account: ChallengeAccountDto; analytics: ReturnType<typeof calculateChallengeAnalytics>; overview: ReturnType<typeof calculateChallengeOverview>; riskStatus: ReturnType<typeof calculateRiskStatus>; trades: ChallengeTradeDto[] }) {
  const [showAll, setShowAll] = useState(false);
  const c = account.accountCurrency;
  const tradingDays = new Set(trades.map((trade) => trade.tradeDate));
  return (
    <section className={styles.panel}>
      <div className={styles.sectionHeader}>
        ...
        <StatusBadge status={riskStatus.status} labels={t.status} />
      </div>
      <div className={styles.kpiHero}>
        <Metric label={t.metrics.status} value={t.status[riskStatus.status]} primary />
        <Metric label={t.metrics.pnl} value={f(overview.runningPnl, c)} tone={overview.runningPnl >= 0 ? 'good' : 'bad'} primary />
        <Metric label={t.metrics.remainingTarget} value={f(overview.remainingTarget, c)} />
        <Metric label={t.metrics.drawdown} value={f(riskStatus.overallDrawdown, c)} tone="bad" />
      </div>
      <button type="button" className={styles.metricsToggle} onClick={() => setShowAll((value) => !value)}>
        {showAll ? t.hideAllMetrics : t.showAllMetrics}
      </button>
      {showAll ? (
        <div className={styles.resultGrid}>
          <Metric label={t.metrics.initial} value={f(account.initialBalance, c)} />
          <Metric label={t.metrics.balance} value={f(account.currentBalance, c)} />
          <Metric label={t.metrics.equity} value={f(account.currentEquity, c)} />
          <Metric label={t.metrics.dailyLoss} value={f(riskStatus.dailyLoss, c)} tone="bad" />
          <Metric label={t.metrics.trades} value={String(analytics.totalTrades)} />
          <Metric label={t.metrics.winRate} value={p(analytics.winRate)} />
          <Metric label={t.metrics.profitFactor} value={Number.isFinite(analytics.profitFactor) ? analytics.profitFactor.toFixed(2) : '∞'} />
          <Metric label={t.metrics.averageRR} value={`${analytics.averageRR.toFixed(2)}R`} />
        </div>
      ) : null}
      <div className={styles.progressGrid}>
        <Progress label={t.metrics.targetProgress} value={overview.targetProgressPct} />
        <Progress label={t.metrics.dailyLoss} value={riskStatus.dailyLossUsagePct} />
        <Progress label={t.metrics.drawdown} value={riskStatus.overallDrawdownUsagePct} />
        <Progress
          label={`${t.metrics.minDays} (${tradingDays.size}/${account.minTradingDays || 0})`}
          value={account.minTradingDays ? (tradingDays.size / account.minTradingDays) * 100 : 100}
        />
      </div>
    </section>
  );
