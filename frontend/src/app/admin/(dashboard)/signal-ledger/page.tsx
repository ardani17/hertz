...
import styles from './signal-ledger.module.css';
...
export default function AdminSignalLedgerPage() {
  ...
  const loadPending = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/admin/signal-ledger/pending', { cache: 'no-store' });
      ...
      setError(err instanceof Error ? err.message : 'Gagal memuat Signal Ledger');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function runAction(postId: string, action: 'publish' | 'reject' | 'hide') {
    ...
    try {
      const response = await fetch(`/api/admin/signal-ledger/posts/${postId}/${action}`, {
        method: 'POST',
  ...
  return (
    <div className={styles.page}>
      <header className={styles.header}>
        <div>
          <p className={styles.kicker}>Signal Ledger</p>
          <h1>Moderasi Feed</h1>
        </div>
      ...
      <section className={styles.metrics} aria-label="Signal Ledger moderation summary">
        <div>
        ...
        <div className={styles.list}>
          {data?.posts.map((post) => (
            <article key={post.id} className={styles.postCard}>
              ...
              <h3>{post.title ?? 'Untitled signal'}</h3>
              <p>{post.body}</p>
